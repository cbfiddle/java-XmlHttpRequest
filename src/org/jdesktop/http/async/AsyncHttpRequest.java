...
/*
 * # load event and onload attribute; # error event and onerror attribute; # progress event and onprogress attribute; #
 * abort event and onabort attribute; # Timers have been suggested, perhaps an ontimeout attribute; # Property to
 * disable following redirects; # responseXML for text/html documents; # property to follow redirects
 */

/**
 * <p>
 * A simple non visual bean that uses HTTP to interact with web servers. This class conforms (except for XML specific
 * functionality) with XMLHttpRequest as defined in the W3C Working Draft Specification
 * <a href="http://www.w3.org/TR/XMLHttpRequest/">http://www.w3.org/TR/XMLHttpRequest/</a>.
 * </p>
 *
 * <p>
 * Several methods, such as the abort() method, cause the AsyncHttpRequest to be "reset". This means that any state
 * associated with the response is cleared, and the readyState reset to UNINITIALIZED.
 * </p>
 *
 * <p>
 * AsyncHttpRequest relies on {@link org.jdesktop.http.Session} for communicating with the server. It is possible to
 * reuse an existing Session. However, if a Session is not specified, a new one is constructed for the request.
 * </p>
 *
 * @author rbair
 */
	...
	protected class AsyncWorker extends SwingWorker {
		...
		protected Object doInBackground() throws Exception {
			try {
				...
				status = response.getStatusCode().getCode();
